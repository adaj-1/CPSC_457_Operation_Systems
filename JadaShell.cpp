...
int system_call(vector<string> *parsed_args, vector<cmds> *redirections)
{
    ...
    if (redirections->size() != 0)  // background execution
    {
        argv.pop_back();    //removes "&" from parameter list
        argv.push_back(nullptr);
    }
    else // not background execution
    {
    ...
    {
        if (redirections->size() != 0) // check if background execution
        {
            cout << pid << endl;
        }
        else
...
int main()
{
    ...
    while (1)
    {
        ...
        if (status == 0)
        {
            ...
            if (redirection_indices.size() == 0) // no io_redirection, pipe, or background called
            {
                status = system_call(&parsed_args, &redirections);
            }
            else
            {              
                multi_cmd(&parsed_args, &redirection_indices, &inputs, &redirections);  //sort commands and arguments
                if (redirections.size() == 1 && redirections.front() == is_background)
                {
                    status = system_call(&parsed_args, &redirections);
                }
